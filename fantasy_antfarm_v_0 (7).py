...
class Simulator:
    ...
    def print_summary(self) -> None:
        ...
        for region_id in world.regions:
            world.evaluate_region_rule(region_id)
        hero, villain = self._pick_top_hero_and_villain()
        top_region = self._top_region()
        top_deity, _, _ = self._top_deity()
        ...
        print(f"Current date: Year {year}, {season}, {MONTH_NAMES[month - 1]} {day}, {tod}")
        ...
        print(self._chronicle_title(hero, top_region, top_deity))
        avg_order = sum(region.order for region in world.regions.values()) / len(world.regions)
        good_regions = len([r for r in world.regions.values() if r.control >= 20])
        evil_regions = len([r for r in world.regions.values() if r.control <= -20])
        contested = len(world.regions) - good_regions - evil_regions
        ...
        if hero is not None:
            print(f"  {self._hero_tale(hero)}")
        if villain is not None:
            print(f"  {self._villain_tale(villain)}")
        print(f"  {state_line}")
        print()

        print("Population by role:")
        living_by_role = {role: 0 for role in Role}
        for actor in world.living_actors():
        ...
        for deity, count, pct in self._deity_influence_summary():
        ...
        for region in world.regions.values():
            ...
            print(f"  {region.name:14} {region.biome:10} danger={region.danger} control={region.control:4} order={region.order:3} lean={leaning:9} ruler={ruler}")
        print()

        print("Monsters still abroad:")
        living_monsters_by_kind = {kind: 0 for kind in MonsterKind}
        for monster in world.living_monsters():
            living_monsters_by_kind[monster.kind] += 1
        for kind in MonsterKind:
        ...
        if world.commemorations:
            ...
            for item in sorted(world.commemorations, key=lambda c: (c.month, c.day, c.name))[:20]:
                ...
                print(f"  {MONTH_NAMES[item.month - 1]} {item.day}: {item.name}{region_text} — {item.reason}")
            print()

        if avg_order >= 60 and good_regions >= evil_regions:
        ...
        survivors = sorted(world.living_actors(), key=lambda actor: (actor.reputation, actor.dragon_kills, actor.horror_kills, actor.monster_kills, actor.kills, actor.power_rating()), reverse=True)
        print("Top surviving adventurers:")
        shown = 0
        for actor in survivors:
            if not actor.is_adventurer():
                continue
            birth = f"{MONTH_NAMES[actor.birth_month - 1]} {actor.birth_day}"
            print(f"  {actor.full_name():28} {actor.role.value:9} {actor.alignment.value:16} {actor.deity.value:16} rep={actor.reputation:2} kills={actor.kills:2} mkills={actor.monster_kills:2} dragons={actor.dragon_kills:2} horrors={actor.horror_kills:2} luck={actor.luck:2} hp={actor.hp:2}/{actor.max_hp:2} birth={birth:15} region={world.region_name(actor.region_id)}")
            shown += 1
            if shown >= 10:
                break
